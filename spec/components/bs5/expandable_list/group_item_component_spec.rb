  ...
  module ExpandableList
    RSpec.describe GroupItemComponent, type: :component do
      ...
      describe 'with a simple title and body' do
        ...
        it {
          is_expected.to have_css(
            'div.list-group-item .expandable-item-header-title:not([aria-expanded="true"])'
          )
        }
        it { is_expected.to have_css('div.list-group-item .expandable-item-body.collapse:not(.show)') }

        it {
          is_expected.to have_css('div.list-group-item.expandable-item .expandable-item-body.collapse:not(.show)[id]',
      ...
      describe 'that behaves as a stretchable items' do
        ...
        it {
          is_expected.to have_css('.list-group-item[data-controller="stretchable-item"][data-stretchable-item-target="container"][data-stretchable-item-stretched-class="stretched"]')
        }
      ...
      describe 'that is expanded' do
        ...
        let(:options) { { expanded: true } }

        it { is_expected.to have_css('div.list-group-item .expandable-item-header-title[aria-expanded="true"]') }
        it { is_expected.to have_css('div.list-group-item .expandable-item-body.collapse.show') }
      end
    end
  end
end
